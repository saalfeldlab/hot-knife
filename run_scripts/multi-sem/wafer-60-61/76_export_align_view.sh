...
ZARR_OUTPUT="${N5_SURFACE_ROOT}/zarr-export/pass${PADDED_PASS}"

if gcloud storage ls "${N5_PATH}${ZARR_OUTPUT}" 2>/dev/null | grep -q .; then
  echo "ERROR: ${N5_PATH}${ZARR_OUTPUT} already exists"
  exit 1
fi
