...
public class ViewISPIMStackHDF5 implements Callable<Void>, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -5648053725605615784L;
	...
	public static final void main(final String... args) throws IOException, InterruptedException, ExecutionException {
		...
		new CommandLine(new ViewISPIMStackHDF5()).execute(args);
	}
	 ...
	 * @throws IOException
	 */
	public static <T extends NumericType<T> & NativeType<T>> ValuePair<RealRandomAccessible<T>, RealInterval> openAlignedStackHDF5(
			final List<Slice> slices,
			final Pair<String, String> hdf5s,
			final T background,
	...
	{
		return openAlignedStackHDF5( slices, hdf5s, background, interpolationMethod, camTransform, alignment, firstSliceIndex, lastSliceIndex, true );
	}
	...
	public static <T extends NumericType<T> & NativeType<T>> ValuePair<RealRandomAccessible<T>, RealInterval> openAlignedStackHDF5(
		...
		/* get slices */
		final ValuePair<List<RealRandomAccessible<T>>, RealInterval> realSlices = openStackHDF5(
				slices,
				hdf5s,
	...
	public static <T extends NumericType<T> & NativeType<T>> ValuePair<List<RealRandomAccessible<T>>, RealInterval> openStackHDF5(
			final List<Slice> slices,
			final Pair<String, String> hdf5s,
		...
		final N5HDF5Reader h5reader = new N5HDF5Reader( hdf5s.getA(), 0, 0, 0 );
		RandomAccessibleInterval img3d = N5Utils.open( h5reader, hdf5s.getB() );

		final ArrayList<RealRandomAccessible<T>> slicesList = new ArrayList<>();
		RealInterval bounds = null;

		for (int i = firstSliceIndex; i <= lastSliceIndex; ++i) {
			final RandomAccessibleInterval<T> img = Views.hyperSlice( img3d, 2, i );

			if (bounds == null)
				bounds = new FinalInterval( img );
			else
	...
	public static <T extends NumericType<T> & NativeType<T>> Pair< RealRandomAccessible<T>, Interval > prepareCamSourceHDF5(
			final List<Slice> slices,
			final Pair<String, String> hdf5s,
			final T background,
			final Interpolation interpolationMethod,
	...
	protected static <T extends NumericType<T> & NativeType<T>> BdvStackSource<T> showCamSourceHDF5(
		...
		final Pair< RealRandomAccessible<T>, Interval > data = prepareCamSourceHDF5(slices, hdf5s, background, interpolationMethod, camTransform, transform, alignment, firstSliceIndex, lastSliceIndex);
	...
	private static <T extends NumericType<T> & NativeType<T>> BdvStackSource<?> runHDF5(
			final HashMap<String, HashMap<String, AffineTransform2D>> camTransforms,
			final AffineGet transform,
			final HashMap<String, HashMap<String, List<Slice>>> stacks,
			final HashMap<String, HashMap<String, Pair<String, String>>> hdf5s,
		...
		for (final Entry<String, HashMap<String, List<Slice>>> channel : stacks.entrySet()) {
			...
			for (final Entry<String, List<Slice>> cam : channel.getValue().entrySet()) {
				...
				bdv = showCamSourceHDF5(
						bdv,
						title,
						cam.getValue(),
						hdf5s.get( channel.getKey() ).get( cam.getKey() ),
	...
	public Void call() throws IOException, InterruptedException, ExecutionException, FormatException {
		...
		final HashMap<String, HashMap<String, Pair<String, String>>> hdf5s = new HashMap<>();
		...
		for (final Entry<String, HashMap<String, AffineTransform2D>> channel : camTransforms.entrySet()) {
			...
			final HashMap<String, Pair<String, String>> channelHdf5s = new HashMap<>();
			hdf5s.put(channel.getKey(), channelHdf5s);
			...
			for (final String camKey : channel.getValue().keySet()) {
				...
				if (n5.exists(groupName)) {
					...
					String hdfFile = n5.getAttribute( groupName, "hdf5file", String.class );
					String hdf5dataset = n5.getAttribute( groupName, "hdf5dataset", String.class );
	
					if ( hdfFile == null || hdf5dataset == null )
						System.out.println( "WARNING: HDF5 file and dataset description missing.");
	
					channelHdf5s.put(camKey, new ValuePair<>(hdfFile, hdf5dataset ) );
				}
			}
		}
...
//		final AffineTransform3D stretchTransform = new AffineTransform3D();
		...
		runHDF5(camTransforms, stretchTransform, stacks, hdf5s, alignments, firstSliceIndex, localLastSliceIndex);

		return null;
	}

	public static void mainTest( String args[] ) throws IOException, InterruptedException, ExecutionException, FormatException {
