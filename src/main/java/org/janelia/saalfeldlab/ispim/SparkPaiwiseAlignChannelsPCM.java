...
public class SparkPaiwiseAlignChannelsPCM implements Callable<Void>, Serializable {
			...
			final int numThreads ) throws FormatException, IOException, ClassNotFoundException, InterruptedException, ExecutionException
	{
		...
		for ( int i = 0; i < numBlocks; ++i )
		{
			...
			if ( i != numBlocks - 1 )
			{
				final int from2  = i * blockSize + firstSliceIndex + blockSize / 2;
				final int to2 = Math.min( localLastSliceIndex, from2 + blockSize - 1 );
		...
		List<Tuple2<Block, ArrayList< PointMatch >>> results = pool.submit( () -> blocks.parallelStream().map(block -> {
			...
			try {
				model.fit(matches);
				System.out.println( SparkPaiwiseAlignChannelsGeo.getError( matches, model ) + ", " + SparkPaiwiseAlignChannelsGeo.getMaxError( matches, model ) );
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
