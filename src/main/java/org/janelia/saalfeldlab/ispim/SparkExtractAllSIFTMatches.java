...
public class SparkExtractAllSIFTMatches implements Callable<Void>, Serializable {
	...
	public Void call() throws IOException, InterruptedException, ExecutionException, FormatException {
		...
		final Stream<String[]> channelsCams =
				camTransforms.entrySet().stream().flatMap(
						channelEntry -> channelEntry.getValue().keySet().stream().map(cam -> new String[] {channelEntry.getKey(), cam}));
		final Stream<String[]> idsChannelsCams = ids.stream().flatMap(id -> channelsCams.map(c -> new String[] {id, c[0], c[1]}));
