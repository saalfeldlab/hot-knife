...
/**
 * Inpaints missing values in a 2D/3D image. This was used for the wafer 53 multi-SEM data set, since there were missing
 * regions in the data due to errors in the MFOV positioning during acquisition.
 *
...
public class InpaintMultiSEM {
	...
	private static final int NUM_ORIENTATIONS = 256;
	private static final double RATIO_OF_2D_RAYS = 0.1;

	private static boolean isInside(final RealLocalizable p, final Interval r) {
	...
	/**
	 * Inpaints missing values in a 2D/3D image by casting rays in random directions and averaging the values of the
	...
	public static void inpaint3d(final RandomAccessibleInterval<FloatType> img, final RandomAccessibleInterval<FloatType> out) {
		...
		final RayCaster rayCaster = new RayCaster(iR, RATIO_OF_2D_RAYS, rnd, MAX_INPAINTING_DIAMETER);
	...
	public static void main(String[] args) {
		...
		final Interval interval = Intervals.createMinMax(36000, 42600, 12, 36600, 44100, 16);
	...
	private static class RayCaster {
		 ...
		 * @param rayDirection the direction in which to cast the ray
