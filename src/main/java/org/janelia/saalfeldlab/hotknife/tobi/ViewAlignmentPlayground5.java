...
public class ViewAlignmentPlayground5 {
	...
	public static void main(String[] args) throws IOException {
		...
		final Cursor<DoubleType> cursor = baked.localizingCursor();
		while (cursor.hasNext()) {
			cursor.fwd();
			final double x = cursor.getDoublePosition(0);
			final double y = cursor.getDoublePosition(1);
			final int d = cursor.getIntPosition(2);
...
//							* scale - offsetXY)
		...
		// TODO: new positionField
		//       new TransformedSurfacePyramid with new positionField and identity moving transform
	}
